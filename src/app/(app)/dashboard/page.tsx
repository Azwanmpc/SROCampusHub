  ...
  return (
    <div className="bg-[var(--white)] p-4">
      <div className="mb-2.5 flex items-center justify-between">
        <div className="text-[11px] font-bold uppercase tracking-[0.04em] text-[rgba(var(--ink-rgb),0.6)]">{label}</div>
        {Icon && <Icon weight="duotone" size={20} color={iconColor} />}
      </div>
      <div className="font-archivo text-[26px] font-extrabold">{value}</div>
      {hint && <div className="mt-0.5 text-[11.5px] text-[rgba(var(--ink-rgb),0.55)]">{hint}</div>}
    </div>
  );
}

function CardsGrid({ children }: { children: React.ReactNode }) {
  return <div className="mb-6 grid grid-cols-2 gap-px border border-[rgba(var(--ink-rgb),0.4)] bg-[rgba(var(--ink-rgb),0.4)] md:grid-cols-4">{children}</div>;
}
...
export default async function DashboardPage() {
  ...
  if (isStaff) {
    ...
    return (
      <div>
        ...
        <div className="mb-4 text-[13.5px] text-[rgba(var(--ink-rgb),0.6)]">
        ...
        <div className="grid grid-cols-1 gap-px border border-[rgba(var(--ink-rgb),0.4)] bg-[rgba(var(--ink-rgb),0.4)] lg:grid-cols-2">
          <div className="bg-[var(--white)] p-[18px]">
            ...
            <div className="flex flex-col divide-y divide-[rgba(var(--ink-rgb),0.15)]">
              {recentBookings.length === 0 && <p className="py-2 text-sm text-[rgba(var(--ink-rgb),0.5)]">Tiada tempahan menunggu.</p>}
              {recentBookings.map((b) => (
                <div key={b.id} className="flex items-center justify-between py-2.5">
                  <div>
                    <div className="text-sm font-bold">{b.facility.name}</div>
                    <div className="text-xs text-[rgba(var(--ink-rgb),0.55)]">
          ...
          <div className="bg-[var(--white)] p-[18px]">
            ...
            <div className="flex flex-col divide-y divide-[rgba(var(--ink-rgb),0.15)]">
              {recentComplaints.length === 0 && <p className="py-2 text-sm text-[rgba(var(--ink-rgb),0.5)]">Tiada aduan.</p>}
  ...
  if (session.role === "PEMOHON") {
    ...
    return (
      <div>
        <div className="mb-0.5 font-archivo text-[26px] font-extrabold">Dashboard</div>
        <div className="mb-4 text-[13.5px] text-[rgba(var(--ink-rgb),0.6)]">Selamat kembali, {session.name} — Pemohon</div>
        <div className="mb-[22px] h-0.5 bg-[rgba(var(--ink-rgb),0.4)]" />
        ...
        {upcoming.length > 0 && (
          <div className="mb-6 border border-[rgba(var(--ink-rgb),0.4)] bg-[var(--white)] p-[18px]">
  ...
  if (session.role === "TEKNIKAL") {
    ...
    return (
      <div>
        ...
        <div className="mb-4 text-[13.5px] text-[rgba(var(--ink-rgb),0.6)]">Ringkasan kerja penyelenggaraan, {session.name}</div>
        <div className="mb-[22px] h-0.5 bg-[rgba(var(--ink-rgb),0.4)]" />
        ...
        <div className="border border-[rgba(var(--ink-rgb),0.4)] bg-[var(--white)] p-[18px]">
          ...
          <div className="flex flex-col divide-y divide-[rgba(var(--ink-rgb),0.15)]">
            {complaints.length === 0 && <p className="py-2 text-sm text-[rgba(var(--ink-rgb),0.5)]">Tiada aduan.</p>}
