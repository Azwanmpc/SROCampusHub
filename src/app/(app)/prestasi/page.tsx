...
export default async function PrestasiPage() {
  ...
  const cards = [
    { label: "Selesai", value: selesai.length, color: "var(--stat-selesai)" },
    { label: "Belum Selesai", value: belumSelesai.length, color: "var(--stat-belum-selesai)" },
    { label: "Staff Dalaman", value: staffDalaman.length, color: "var(--stat-staff-dalaman)" },
    { label: "Kontraktor Luar", value: kontraktorLuar.length, color: "var(--stat-kontraktor-luar)" },
  ];
  ...
  return (
    <div>
      ...
      <div className="mb-5 grid grid-cols-2 gap-px border border-[rgba(var(--ink-rgb),0.4)] bg-[rgba(var(--ink-rgb),0.4)] md:grid-cols-4">
        {cards.map((c) => (
          <div key={c.label} className="bg-[var(--white)] p-4">
            <div className="mb-2 text-[12.5px] font-bold uppercase tracking-[0.04em] text-[rgba(var(--ink-rgb),0.6)]">{c.label}</div>
            <div className="font-archivo text-[38px] font-extrabold" style={{ color: c.color }}>
              {c.value}
            </div>
          </div>
        ))}
      ...
      <div className="border border-[rgba(var(--ink-rgb),0.4)] bg-[var(--white)] p-[18px]">
        <div className="mb-3 font-archivo text-base font-extrabold">Untuk Tindakan (Susun Ikut Tempoh Pending)</div>
        <div className="grid grid-cols-1 gap-px border border-[rgba(var(--ink-rgb),0.3)] bg-[rgba(var(--ink-rgb),0.3)] md:grid-cols-2">
          <div className="bg-[var(--white)] p-3.5">
            <div className="mb-2 text-[13px] font-bold uppercase tracking-[0.03em] text-[rgba(var(--ink-rgb),0.6)]">Staf Penyelenggaraan</div>
            {pendingDalaman.length === 0 && <p className="text-[13px] text-[rgba(var(--ink-rgb),0.5)]">Tiada aduan pending.</p>}
            {pendingDalaman.map((c) => (
              <div key={c.id} className="flex items-center gap-3 border-b border-[rgba(var(--ink-rgb),0.15)] py-2.5 last:border-0">
                <div className="flex-1">
                  <div className="text-[15px] font-bold">{c.location}</div>
                  <div className="text-[13px] font-semibold text-[rgba(var(--ink-rgb),0.6)]">{c.description}</div>
                </div>
                <div className="text-[13.5px] font-bold" style={{ color: c.hariPending > 2 ? "var(--danger)" : "var(--warning)" }}>
                  {c.hariPending} hari
                </div>
              </div>
            ))}
          ...
          <div className="bg-[var(--white)] p-3.5">
            <div className="mb-2 text-[13px] font-bold uppercase tracking-[0.03em] text-[rgba(var(--ink-rgb),0.6)]">Kontraktor Luar</div>
            {pendingKontraktor.length === 0 && <p className="text-[13px] text-[rgba(var(--ink-rgb),0.5)]">Tiada aduan pending.</p>}
