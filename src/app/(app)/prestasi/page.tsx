...
export default async function PrestasiPage() {
  ...
  return (
    <div>
      ...
      <div className="mb-3.5 text-[13.5px] text-[rgba(32,30,29,0.6)]">Jejak prestasi dalaman vs kontraktor</div>
      <div className="mb-5 h-0.5 bg-[rgba(32,30,29,0.4)]" />

      <div className="mb-5 grid grid-cols-2 gap-px border border-[rgba(32,30,29,0.4)] bg-[rgba(32,30,29,0.4)] md:grid-cols-4">
        {cards.map((c) => (
          <div key={c.label} className="bg-white p-4">
            <div className="mb-2 text-[11px] font-bold uppercase tracking-[0.04em] text-[rgba(32,30,29,0.6)]">{c.label}</div>
      ...
      <div className="border border-[rgba(32,30,29,0.4)] bg-white p-[18px]">
        ...
        <div className="grid grid-cols-1 gap-px border border-[rgba(32,30,29,0.3)] bg-[rgba(32,30,29,0.3)] md:grid-cols-2">
          <div className="bg-white p-3.5">
            <div className="mb-2 text-xs font-bold uppercase tracking-[0.03em] text-[rgba(32,30,29,0.6)]">Staf Penyelenggaraan</div>
            {pendingDalaman.length === 0 && <p className="text-xs text-[rgba(32,30,29,0.5)]">Tiada aduan pending.</p>}
            {pendingDalaman.map((c) => (
              <div key={c.id} className="flex items-center gap-3 border-b border-[rgba(32,30,29,0.15)] py-2.5 last:border-0">
                <div className="flex-1">
                  <div className="text-[13px] font-bold">{c.location}</div>
                  <div className="text-[11.5px] text-[rgba(32,30,29,0.6)]">{c.description}</div>
                </div>
          ...
          <div className="bg-white p-3.5">
            <div className="mb-2 text-xs font-bold uppercase tracking-[0.03em] text-[rgba(32,30,29,0.6)]">Kontraktor Luar</div>
            {pendingKontraktor.length === 0 && <p className="text-xs text-[rgba(32,30,29,0.5)]">Tiada aduan pending.</p>}
