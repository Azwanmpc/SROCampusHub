...
export default function FacilityCard({
  ...
  return (
    <div className="flex h-full flex-col border border-[rgba(var(--ink-rgb),0.3)] bg-[var(--white)]">
      {f.imageUrl ? (
        // eslint-disable-next-line @next/next/no-img-element
        <img src={f.imageUrl} alt={f.name} className="h-[110px] w-full object-cover" />
      ) : (
        <div className="flex h-[110px] items-center justify-center bg-[repeating-linear-gradient(135deg,#e7e5e5_0_10px,#d7d3d3_10px_20px)] text-xs font-semibold text-[rgba(var(--ink-rgb),0.4)]">
          Gambar {f.name}
        </div>
      )}
      <div className="flex flex-1 flex-col p-4">
        ...
        <div className="mb-2 text-[12.5px] text-[rgba(var(--ink-rgb),0.6)]">{f.description}</div>
        ...
        {isAsrama ? (
          <div className="mb-3 bg-[var(--surface)] px-2.5 py-2">
            <div className="text-[9.5px] font-bold uppercase tracking-[0.03em] text-[rgba(var(--ink-rgb),0.55)]">
              Kadar Bilik
            </div>
          ...
          <div className="mb-3 flex gap-px border border-[rgba(var(--ink-rgb),0.15)] bg-[rgba(var(--ink-rgb),0.15)]">
            <div className="flex-1 bg-[var(--surface)] px-2.5 py-2">
              <div className="text-[9.5px] font-bold uppercase tracking-[0.03em] text-[rgba(var(--ink-rgb),0.55)]">
                Separuh Hari
              </div>
